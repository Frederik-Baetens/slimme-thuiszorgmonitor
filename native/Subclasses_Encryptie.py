# De S-Blok
s_blok = bytearray((0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76,
...
inv_s_blok =  bytearray((0x52, 0x09, 0x6A, 0xD5, 0x30, 0x36, 0xA5, 0x38, 0xBF, 0x40, 0xA3, 0x9E, 0x81, 0xF3, 0xD7, 0xFB,
...
def MakeCTR(number):
    ...
    for position in range(4):
        matrix.append(lijst[position*4:(position+1)*4])

    return matrix

@micropython.native
def SubBytes(matrix, s_blok):
    """
    ...
    for position in range(16):
        matrix[position//4][position % 4] = s_blok[matrix[position//4][position % 4]]
    return matrix

@micropython.native
def InvSubBytes(matrix, inv_s_blok):
    """
    ...
    """
    for position in range(16):
        matrix[position//4][position % 4] = inv_s_blok[matrix[position//4][position % 4]]
    return matrix
...
def MixColumns(matrix):
    ...
    for place_x in range(4):
        ...
        mixColumn(column, vermenigvuldigen)
...
@micropython.native
def mixColumn(column, vermenigvuldigen):
    """
...
def AddRoundKey(matrix, key, i):
    ...
    for row in range(4):
        for column in range(4):
            matrix[row][column] ^= key[row][column+i*4]
    return matrix
...
def ImplementMessage(Encryption, Message):
    ...
    for row in range(4):
        for column in range(4):
            Encryption[row][column] ^= Message[row][column]
