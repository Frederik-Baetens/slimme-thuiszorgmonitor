...
def ReadBlok(matrix):
    lijst = []
    for row in range(4):
        lijst += [matrix[row][y] for y in range(4)]
    return lijst
...
def BlokAsListToList(List):
    result = []
    for position in range(4):
        result.append(List[position*3]+(List[position*3+1]//16)*256)
        result.append(List[position*3+2]+(List[position*3+1]%16)*256)

    result += [List[12+i] for i in range(4)]
